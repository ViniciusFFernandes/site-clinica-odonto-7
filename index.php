<?php
// index.php — Landing page da Dr(a). Nome Sobrenome (conteúdo vem de config/clinica.php)
...
$ldjson = json_encode([
  ...
  '@graph' => [[
    ...
    'availableService' => array_map(fn($s) => ['@type' => 'MedicalProcedure', 'name' => $s['title']], $cfg['servicos']),
  ]],
...
<body class="min-h-screen bg-background font-sans text-foreground antialiased">
  ...
  <header id="siteHeader" class="fixed inset-x-0 top-0 z-50 transition-all duration-300 py-5">
    <div class="mx-auto grid max-w-7xl grid-cols-[minmax(0,1fr)_auto] items-center gap-4 px-5 lg:px-8">
      <a href="#topo" class="flex min-w-0 items-center gap-3">
        ...
        <span class="min-w-0">
          <span class="block truncate font-display text-lg font-600 leading-tight text-primary"><?= e($cfg['short']) ?></span>
    ...
    <nav id="mobileMenu" class="mx-5 mt-3 hidden rounded-3xl border border-border bg-background p-4 shadow-[var(--shadow-soft)] lg:hidden">
      ...
      <?php endforeach; ?>
      <a href="<?= e($waLink) ?>" target="_blank" rel="noopener noreferrer" class="mt-1 block rounded-2xl bg-whatsapp px-4 py-3 text-center text-sm font-semibold text-whatsapp-foreground" data-close-menu>Agendar pelo WhatsApp</a>
    </nav>
  </header>
  ...
  <main id="topo">
    ...
    <section class="relative overflow-hidden pt-32 pb-16 lg:pt-40 lg:pb-24" style="background: var(--gradient-soft)">
      ...
      <div class="relative mx-auto grid max-w-7xl items-center gap-12 px-5 lg:grid-cols-2 lg:gap-16 lg:px-8">
        <div class="reveal">
          <span class="inline-flex items-center gap-2 rounded-full border border-gold/40 bg-background px-4 py-2 text-xs font-semibold tracking-wide text-primary uppercase">
            <?= icon('star', 'size-4 fill-gold text-gold') ?>
            <?= e($cfg['rating']) ?> de avaliação · <?= e($cfg['reviews']) ?> avaliações
          </span>
          <h1 class="mt-6 font-display text-5xl leading-[1.05] font-bold tracking-tight text-primary sm:text-6xl lg:text-7xl">
            <?= e($cfg['hero_titulo_1']) ?> <span class="text-accent"><?= e($cfg['hero_titulo_2']) ?></span>
          </h1>
          ...
          <div class="mt-9 flex flex-col gap-3 sm:flex-row">
            ...
            <a href="#tratamentos" class="inline-flex items-center justify-center gap-2 rounded-full border border-border bg-background px-7 py-4 text-base font-semibold text-primary transition-colors hover:bg-secondary">
              <?= icon('arrow-right', 'size-5') ?>Ver tratamentos
            </a>
          </div>

          <div class="mt-8 flex items-center gap-3">
            <div class="flex gap-1 text-gold"><?php for ($k=0;$k<5;$k++) echo icon('star','size-5 fill-gold'); ?></div>
            <span class="text-sm text-muted-foreground"><strong class="text-primary"><?= e($cfg['rating']) ?> de avaliação</strong> · <?= e($cfg['reviews']) ?> avaliações</span>
          </div>
        </div>
        ...
        <div class="reveal relative" style="transition-delay:120ms">
          <div class="relative overflow-hidden rounded-[2.5rem] shadow-[var(--shadow-lift)]">
            <img src="img/hero-dentista.svg" alt="Dr(a). Nome Sobrenome sorrindo durante atendimento odontológico em Cidade Exemplo" width="1200" height="1408" class="h-full w-full object-cover" />
          </div>
          <div class="absolute -bottom-6 left-4 flex items-center gap-3 rounded-3xl border border-border bg-background/95 px-5 py-4 shadow-[var(--shadow-soft)] backdrop-blur sm:left-8">
            ...
            <span class="min-w-0">
              ...
              <span class="block text-xs text-muted-foreground">Cuidado e segurança em cada consulta</span>
            </span>
          </div>
        </div>
      </div>
    ...
    <section id="diferenciais" class="border-y border-border bg-secondary/60">
      <div class="mx-auto grid max-w-7xl gap-6 px-5 py-8 sm:grid-cols-2 lg:grid-cols-4 lg:px-8">
        <?php foreach ($cfg['diferenciais'] as $r): ?>
          <div class="flex min-w-0 items-center gap-3">
            <span class="grid size-11 shrink-0 place-items-center rounded-2xl bg-background text-accent shadow-[var(--shadow-soft)]"><?= icon($r['icon'], 'size-5') ?></span>
            <span class="min-w-0">
              <span class="block text-sm font-semibold text-primary"><?= e($r['title']) ?></span>
              <span class="block text-xs text-muted-foreground"><?= e($r['desc']) ?></span>
            </span>
          </div>
        <?php endforeach; ?>
      </div>
    ...
    <!-- Tratamentos -->
    <section id="tratamentos" class="mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-28">
      <div class="reveal mx-auto max-w-2xl text-center">
        <span class="text-sm font-semibold tracking-widest text-gold uppercase">Tratamentos</span>
        <h2 class="mt-3 font-display text-4xl font-bold tracking-tight text-primary sm:text-5xl">Cuidados completos para o seu sorriso</h2>
        <p class="mt-4 text-lg text-muted-foreground">Do preventivo ao estético: cada caso é planejado individualmente, com orçamento apresentado antes de iniciar.</p>
      </div>

      <div class="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        <?php foreach ($cfg['servicos'] as $i => $s): ?>
          <div class="reveal" style="transition-delay:<?= ($i % 3) * 90 ?>ms">
            <article class="surface-card group flex h-full flex-col p-7">
              <span class="grid size-12 place-items-center rounded-2xl bg-accent-soft text-accent transition-colors group-hover:bg-accent group-hover:text-accent-foreground"><?= icon($s['icon'], 'size-6') ?></span>
              <h3 class="mt-5 font-display text-xl font-semibold text-primary"><?= e($s['title']) ?></h3>
              <p class="mt-2 flex-1 text-sm leading-relaxed text-muted-foreground"><?= e($s['desc']) ?></p>
              <a href="<?= e(whats($cfg, 'Olá, Dr(a). Nome Sobrenome! Gostaria de saber mais sobre: ' . $s['title'] . '.')) ?>" target="_blank" rel="noopener noreferrer" class="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-accent transition-colors hover:text-primary">
                Saiba mais <?= icon('arrow-right', 'size-4') ?>
              </a>
            </article>
          </div>
        <?php endforeach; ?>
      ...
      <div class="reveal mt-12 text-center">
        <a href="<?= e(whats($cfg, 'Olá, Dr(a). Nome Sobrenome! Gostaria de tirar uma dúvida sobre o meu caso.')) ?>" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 rounded-full bg-primary px-7 py-4 text-base font-semibold text-primary-foreground transition-transform hover:scale-[1.02]">
    ...
    <section id="sobre" class="bg-secondary/50 py-20 lg:py-28">
      <div class="mx-auto grid max-w-7xl items-center gap-12 px-5 lg:grid-cols-2 lg:gap-16 lg:px-8">
        <div class="reveal">
          <div class="overflow-hidden rounded-[2.5rem] shadow-[var(--shadow-lift)]">
            <img src="img/clinica-interior.svg" alt="Consultório odontológico da Dr(a). Nome Sobrenome em Cidade Exemplo" loading="lazy" width="1200" height="912" class="h-full w-full object-cover" />
          </div>
        </div>
        <div class="reveal" style="transition-delay:100ms">
          <span class="text-sm font-semibold tracking-widest text-gold uppercase">Sobre a Dr(a). Nome Sobrenome</span>
          <h2 class="mt-3 font-display text-4xl font-bold tracking-tight text-primary sm:text-5xl"><?= e($cfg['sobre_titulo']) ?></h2>
          ...
          <a href="<?= e($waLink) ?>" target="_blank" rel="noopener noreferrer" class="mt-9 inline-flex items-center gap-2 rounded-full bg-whatsapp px-7 py-4 text-base font-semibold text-whatsapp-foreground shadow-[var(--shadow-soft)] transition-transform hover:scale-[1.02]">
            <?= icon('message-circle', 'size-5') ?>Falar com a Dr(a). Nome Sobrenome
          </a>
        </div>
      </div>
    </section>
    ...
    <section class="mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-28">
      <div class="reveal mx-auto max-w-2xl text-center">
        <span class="text-sm font-semibold tracking-widest text-gold uppercase">Como funciona</span>
        <h2 class="mt-3 font-display text-4xl font-bold tracking-tight text-primary sm:text-5xl">Simples do primeiro contato ao tratamento</h2>
      </div>
        ...
        <?php foreach ($cfg['passos'] as $i => $s): ?>
          <div class="reveal" style="transition-delay:<?= $i * 110 ?>ms">
            <div class="relative h-full overflow-hidden rounded-[2rem] border border-border bg-card p-8">
              ...
              <h3 class="relative mt-5 font-display text-xl font-semibold text-primary"><?= e($s['title']) ?></h3>
    ...
    <!-- Galeria -->
    <section id="galeria" class="bg-secondary/50 py-20 lg:py-28">
      <div class="mx-auto max-w-7xl px-5 lg:px-8">
        <div class="reveal mx-auto max-w-2xl text-center">
          <span class="text-sm font-semibold tracking-widest text-gold uppercase">Galeria</span>
          <h2 class="mt-3 font-display text-4xl font-bold tracking-tight text-primary sm:text-5xl">Um ambiente pensado para o seu conforto</h2>
          <p class="mt-4 text-lg text-muted-foreground">Consultório, equipamentos e recepção preparados para uma experiência tranquila e acolhedora.</p>
        </div>

        <div class="mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <?php foreach ($cfg['galeria'] as $i => $g): ?>
            <div class="reveal" style="transition-delay:<?= ($i % 3) * 90 ?>ms">
              <div class="group relative aspect-[4/3] overflow-hidden rounded-[1.75rem] border border-border bg-muted shadow-[var(--shadow-soft)]">
                <img src="<?= e($g['img']) ?>" alt="<?= e($g['alt']) ?>" loading="lazy" class="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" />
              </div>
            </div>
          <?php endforeach; ?>
        </div>
    ...
    <!-- Depoimentos -->
    <section id="depoimentos" class="mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-28">
      <div class="reveal mx-auto max-w-2xl text-center">
        <span class="text-sm font-semibold tracking-widest text-gold uppercase">Depoimentos</span>
        <h2 class="mt-3 font-display text-4xl font-bold tracking-tight text-primary sm:text-5xl">Mais de 100 pacientes avaliaram positivamente</h2>
        <p class="mt-4 text-lg text-muted-foreground">Nota <?= e($cfg['rating']) ?> de 5. Veja o que dizem quem já cuidou do sorriso com a Dr(a). Nome Sobrenome.</p>
      </div>
    ...
    <section class="relative overflow-hidden py-20 lg:py-28" style="background: var(--gradient-deep)">
      <div aria-hidden="true" class="pointer-events-none absolute -bottom-32 -left-24 size-[30rem] rounded-full bg-accent/20 blur-3xl"></div>
      <div class="relative mx-auto max-w-3xl px-5 text-center lg:px-8">
        <h2 class="font-display text-4xl font-bold tracking-tight text-primary-foreground sm:text-5xl"><?= e($cfg['cta_titulo']) ?></h2>
        <p class="mx-auto mt-5 max-w-xl text-lg text-primary-foreground/80"><?= e($cfg['cta_sub']) ?></p>
        <a href="<?= e($waLink) ?>" target="_blank" rel="noopener noreferrer" class="mt-10 inline-flex items-center gap-3 rounded-full bg-whatsapp px-9 py-5 text-lg font-semibold text-whatsapp-foreground shadow-[var(--shadow-lift)] transition-transform hover:scale-[1.03]">
    ...
    <section id="localizacao" class="mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-28">
      <div class="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:gap-14">
        <div class="reveal">
          <span class="text-sm font-semibold tracking-widest text-gold uppercase">Localização</span>
          <h2 class="mt-3 font-display text-4xl font-bold tracking-tight text-primary sm:text-5xl">Fácil de chegar, em <?= e($cfg['city']) ?></h2>
        ...
        <div class="reveal" style="transition-delay:100ms">
          <div class="overflow-hidden rounded-[2rem] border border-border shadow-[var(--shadow-soft)]">
            <iframe title="Mapa — Consultório da Dr(a). Nome Sobrenome" src="https://www.google.com/maps?q=<?= $mapsQ ?>&output=embed" loading="lazy" referrerpolicy="no-referrer-when-downgrade" class="h-[420px] w-full border-0"></iframe>
          </div>
        </div>
      </div>
    </section>
  ...
  <footer class="border-t border-border bg-secondary/60">
    <div class="mx-auto grid max-w-7xl gap-10 px-5 py-14 sm:grid-cols-2 lg:grid-cols-4 lg:px-8">
      <div>
        <span class="font-display text-xl font-semibold text-primary"><?= e($cfg['name']) ?></span>
      ...
      <div>
        ...
        <ul class="mt-4 space-y-2 text-sm text-muted-foreground">
          ...
          <li><a href="<?= e($cfg['instagram']) ?>" target="_blank" rel="noopener noreferrer" class="hover:text-primary">Instagram: @</a></li>
        </ul>
      </div>
      <div>
        ...
        <p class="mt-2 text-sm text-muted-foreground"><?= e($cfg['horario_semana']) ?><br /><?= e($cfg['horario_sabado']) ?></p>
      </div>
      <div>
